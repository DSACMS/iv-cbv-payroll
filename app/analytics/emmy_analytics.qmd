...
1.  How many applicants are starting but not completing the process?
    1.  Is there a drop in completion as the pilot period draws on?
    2.  Is there a difference in completion based on their device or how they receive the pilot link?
2.  How long does it take to complete the application?
    1.  How long does the syncing take?
    2.  When there was a provider outage, was there any change in the processing times?
3.  Where in the process are applicants dropping off?
    1.  Are applicants accessing the documentation (help or FAQ) before dropping off?
    2.  How many applicants are not finding their employer in the search?
    3.  Are the popular app employers the right ones (i.e. do there appear to be employers searched more frequently than any that appear in the popular links)
4.  What share of applicants are return applicants? (only tokenized link applicants)
...
5.  How many times does an applicant "browser back"?
...
When looking the link/url origin, we see the large majority arise from SMS messages. However, the success rate is much lower than that of physical mailings.
...
As mentioned above, we can see that many employers are not being captured in this process. If we break down where that is occuring, we can see it is almost exclusively coming from Pinwheel. 
...
##  What share of applicants are return applicants? (tokenized applicants)

After the May 2025 pilot, the team implemented changes to allow for better tracking of applicants, allowing us to see if the same users were returning to the application. Ideally, if this application were incredibly easy to complete, we would see most using completing in one session. In the histogram below, we can see that the November 2025 had far mor repeated users. 
