...
In looking at the most recent pilot data, we see that of the `r scales::label_comma()(nrow(df_started_w))` applicants who started the application, `scales::label_percent()(df_started_status[df_started_status$status == "Opened and closed",]$share)` opened the link and closed it without proceeding further in their first session. For the applicants who simply opened an closed,  `scales::label_percent()(v_end)` did not attempt to complete the application again. A small number of applications completed the process in their first session (`scales::label_percent()(df_started_status[df_started_status$status == "Opened and finished",]$share)`) and of those who didn't complete it the first time, the median applicant was completing the process in session `r v_sess_complete`.
