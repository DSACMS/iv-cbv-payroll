---
title: "EMMY Mixpanel Web Analytics Review"
date: now
...
2.  The employer search functionality is a critical bottleneck
    -   Issue - about 19% of employer searches return "missing" results across all three pilot periods (May, Aug, Nov 2025), with 34% of applicants dropping off at this step in November
...
3.  Help documentation usage patterns suggest possible UX confusion
    -   Issue - Applicants who completed the application had slightly more "help actions" than those who didn't, with significantly higher help usage on the agreement page among those who dropped off
...
4.  Seeing a higher completion rate for those starting from a mailed link
    ...
    -   Why it matters - The app is designed to be mobile-friendly, so understanding device-based completion rates is critical for optimizing the user experience
5.  Need for investigation of multi-session users
...
default_caption <- "Source: Louisiana 2025 EMMY Pilot Mixpanel Data [accessed 2025-12-19]"
...
We should take a closer look at the repeated submissions. Some of these could be re-submissions of the same information (i.e. the applicant didn't know if the submission went through or forgot they already submitted it) or could be going back to enter in a different employer at a different time. If we look at the `r sum(df_mult_types$n)` applicants with repeated submissions in the November pilot, `r df_mult_types[df_mult_types$behavior == 'Single item repeated',]$n` submitted for the same employer each time, `r df_mult_types[df_mult_types$behavior == 'Multiple items with repeats',]$n` submitted different employers (some with repeats), and only `r df_mult_types[df_mult_types$behavior == 'All unique items',]$n` had all unique employer submissions. The table below provides some examples of how the different behaviors might look like in the data.
...
tribble(
  ...
  si_gt_base(header_fill = dsac_color['light_teal']) |> 
  add_labs(
    title = "Example Applicant Behavior" |> toupper(),
    caption = glue("Note: dummy data")
  ) 
...
If we look at just event data, we can see that virtually all of the submissions are occuring within the first three weeks. 

```{r}
#| label: cumshare
#| output: true

df_compl_cumshare <- df_mp |> 
  filter(event %in% c("ApplicantViewedAgreement","ApplicantSharedIncomeSummary")) |> 
  clean_events() |> 
  group_by(pilot) |> 
  mutate(
    plt_day = as_date(timestamp) - min(as_date(timestamp)),
    plt_day = as.integer(plt_day)
  ) |> 
  group_by(pilot, plt_day, event_clean) |> 
  summarise(
    applicants = n_distinct(distinct_id),
    events = n(),
    .groups = "drop"
...
df_compl_cumshare <- df_compl_cumshare |> 
  group_by(pilot, event_clean) |> 
  complete(
    plt_day = seq(0, max(plt_day)), 
    fill = list(applicants = 0, events = 0)) |> 
  ungroup()

df_compl_cumshare <- df_compl_cumshare |> 
  pivot_longer(c(applicants, events), names_to = "denom") |> 
  arrange(denom, event_clean, pilot, plt_day) |> 
  group_by(pilot, denom, event_clean) |> 
  mutate(
    cumsum = cumsum(value),
    cumshare = cumsum / sum(value, na.rm = TRUE)
  ) |> 
  mutate(
    data_pt = case_when(plt_day %% 7 == 0 ~ plt_day),
    data_lab = case_when(plt_day %% 7 == 0 ~ label_percent(1)(cumshare))) |> 
  filter(denom == "events")
 
df_compl_cumshare |> 
  ggplot(aes(plt_day, cumshare, fill = event_clean)) +
  geom_ribbon(aes(ymin = 0, ymax = cumshare), alpha = .4) +
  geom_step(color = matterhorn) +
  geom_vline(xintercept = 21, color = "white") +
  geom_text(
    aes(label = data_lab), na.rm = TRUE,
  ...
  facet_grid(fct_rev(event_clean)~pilot, switch = "y") +
  scale_x_continuous(
    breaks = seq(0, max(df_compl_cumshare$plt_day), by = 7),
    labels = seq(0, max(df_compl_cumshare$plt_day), by = 7)/ 7
  ) +
  scale_fill_manual(values = c(unname(dsac_color['light_teal']), unname(dsac_color["light_navy"]))) +
  scale_y_continuous(label = label_percent()) +
  labs(
    x = NULL, y = NULL,
    title = glue("97% OF <span style = color:{dsac_color['light_teal']};>SHARED INCOME SUMMARY</span> EVENTS OCCUR WITHIN 3 WEEKS"),
    subtitle = "Cumulative share of **events** by week | Louisiana Pilots",
...
For the Louisiana pilot, applicants were both mailed paper notices and sent an SMS with a notice about the pilot. We can investigate whether there is any difference in completion rates based on how the applicants are accessing the app.

::: callout-note
The origin, or invitation to start the application, comes through either a paper notice or SMS.

Before the November 2025 Louisiana pilot, both notices provided applicants with a generic link with their origin type, "mail" or "sms".

Currently, the origin is determined as follows:

1.  if an applicant uses a custom link used on their notices, the `origin` is set to "mail";

2.  if the link is shared from another applicant (or through the MFB in the 2025 Arizona pilot) a `?origin` parameter when the flow is started, the origin is determined to be "shared"; or

3.  when no `origin` parameter is used, Mixpanel interprets this as the "default origin", which is agency configured and set it to "sms" for the Louisiana pilot.
:::
...
When comparing the device type, we can see there is a preference for applicants initiating the process on their phones. The success rate has been higher on the phones than desktop, but both success rates are rather low. Interestingly there has been a drop each pilot in desktop completion rates and in November, there was actual a gain in smartphone completion rate, furthering the divide. The other types of devices include `{r} other_devices`. Each pilot also see a number of unknown devices. Looking at the sessions for these unknown devices, they almost exclusively are just clicking the link to start and viewing the agreement page and not going beyond that.
...
The table below so similar drop off rates at each step for each of the three pilots in Louisiana.
...
df_ret |>
  ...
  sub_missing() |> 
  add_labs(
    title = "LARGE DROP IN GETTING TO SECOND PAGE" |> toupper(),
    caption = default_caption
  ) 
...
As mentioned above, we can see that many employers are not being captured in this process. If we break down where this is occurring, we can see it is almost exclusively coming from Pinwheel. In the most November pilot, `r label_percent()(df_missing_empl[df_missing_empl$provider == "Pinwheel" & df_missing_empl$pilot == "Nov 2025",]$share_missing)` of the synced data were missing an employer name.
...
#| output: true
 ...
 df_missing_empl|> 
  ...
  add_labs(
    title = "Half of Pinwheel sync appear to not capture the employer" |> toupper(),
    caption = default_caption
  ) 
```
...
In an ideal word, user would receive their link to complete the income verification and complete it shortly there after in one go. What we see from the data is that many people open the link to determine what they need to do and come back later to try and complete it.
...
df_started <- df_mp |> 
  filter(
    pilot == latest_pilot,
    event %in% c("ApplicantClickedCBVInvitationLink", "ApplicantViewedAgreement", "ApplicantAgreed", "ApplicantSharedIncomeSummary")
    ) |>
  mutate(
    event = factor(event, c("ApplicantClickedCBVInvitationLink","ApplicantViewedAgreement", "ApplicantAgreed", "ApplicantSharedIncomeSummary")),
    status = 1L
  ) |> 
  distinct(pilot, distinct_id, cbv_flow_id, event, status, seconds_since_invitation) 

df_started_w <- df_started |>
  group_by(cbv_flow_id) |> 
  fill(seconds_since_invitation, .direction = "downup") |> 
  ungroup() |> 
  pivot_wider(
    names_from = event,
    values_from = status,
  ...
  relocate(ApplicantClickedCBVInvitationLink,ApplicantViewedAgreement, ApplicantAgreed, ApplicantSharedIncomeSummary, .after = seconds_since_invitation) |> 
  group_by(distinct_id) |> 
  mutate(
    ever_finished = max(ApplicantSharedIncomeSummary, na.rm = TRUE),
    session = row_number() 
    ) |> 
  ungroup() |> 
  mutate(
    first_session_complete = ifelse(ApplicantSharedIncomeSummary == 1, session, 99),
    status = case_when(
      ApplicantAgreed == 0 ~ "Opened and closed",
      ApplicantSharedIncomeSummary == 1 ~ "Opened and finished",
      ApplicantAgreed == 1 ~ "Opened and started, but didn't finish",
  )) 

df_started_w <- df_started_w |> 
  group_by(distinct_id) |> 
  mutate(
    session = max(session),
    first_session_complete = min(first_session_complete, na.rm = TRUE)
    ) |> 
  slice_min(order_by = cbv_flow_id, n = 1) |> 
  ungroup() |> 
  mutate(first_session_complete = ifelse(first_session_complete == 99, NA, first_session_complete))

df_started_w |> 
  count(ApplicantClickedCBVInvitationLink, status, ever_finished) |> 
  pivot_wider(names_from = ever_finished, values_from = n)

df_started_status <- df_started_w |> 
  count(pilot, status) |> 
  mutate(share = n / sum(n))

 df_started_status_ever <- df_started_w |> 
  count(pilot, status, ever_finished) |> 
  pivot_wider(names_from = ever_finished, values_from = n) |> 
  rowwise() |> 
  mutate(share = `1` / sum(`0`, `1`))

v_end <- df_started_w |> 
  filter(status == "Opened and closed") |> 
  count(pilot, status, session == 1) |> 
  mutate(share = n / sum(n)) |> 
  filter(`session == 1` == TRUE) |> 
  pull()

v_sess_complete <- df_started_w |> 
  filter(
    # status == "Opened and started, but didn't finish",
    !first_session_complete %in% c(NA, 1)) |> 
  pull(first_session_complete) |> 
  median(na.rm = TRUE)

```

In looking at the most recent pilot data, we see that of the `r scales::label_comma()(nrow(df_started_w))` applicants who started the application, `r scales::label_percent()(df_started_status[df_started_status$status == "Opened and closed",]$share)` opened the link and closed it without proceeding further in their first session. For the applicants who simply opened an closed, `r scales::label_percent()(v_end)` did not attempt to complete the application again. A small number of applications completed the process in their first session (`r scales::label_percent()(df_started_status[df_started_status$status == "Opened and finished",]$share)`) and of those who didn't complete it the first time, the median applicant was completing the process in session `r v_sess_complete`.

```{r}
#| label: first_opened_viz
#| output: true

df_started_status |> 
  ggplot(aes(share, pilot, fill = status)) +
  geom_col() +
  geom_text(aes(label = label_percent()(share)), family = "Source Sans 3 Semibold", color = matterhorn, hjust = -.3) +
  facet_wrap(~fct_reorder(status, share, .desc = TRUE), ncol = 1) +
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, .25)) +
  scale_fill_manual(
    values = 
      c(
        "Opened and closed" = unname(dsac_color['cranberry']), 
        "Opened and started, but didn't finish" = unname(dsac_color['light_cranberry']), 
        "Opened and finished" = unname(dsac_color['light_navy']))) +
  si_style_xgrid() +
  labs(
    x = NULL, y = NULL,
    title = "UPON RECEIVING THE INVITATIONS, APPLICANTS LARGELY CLICKED THE LINK AND DID NOT PROCEED" |> str_wrap(70),
    subtitle = "Share of applicants responses for the first session after receiving the CBV invitation" |> str_wrap(70),
    caption = glue("Note: Excluded generic link starts
    {default_caption}"
    )
  ) +
  theme(
