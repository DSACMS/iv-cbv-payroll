...
df_mp |> 
  ...
  adjust_row_padding() |> 
  add_labs(
    title = "Some users had multiple successful submissions" |> toupper(),
    subtitle = "the number of successful submission sessions",
    caption = default_caption
  ) 
```
...
Sticking with the provider comparision, we can look at the distribtion of sync times. By bucketing them, we can see that most sessions have a sync time between 30 seconds and a minute and about 95% are under the 3 minute mark. 
...
The next visual look at the same data, but plots each of the sessions individually, allowing us to see some of the larger outliers and where the specific times fall compared within the broader buckets. The x-axis has been truncated to 20 minutes to see the distrbution, plotting the `r filter(df_sync, sync_time > 1200) |> nrow()` outliers on the 20 minute mark. Both providers saw three sessions with sync times over 20 minutes in the November pilot, but the median time went down from August.  
