...
-   **Application submitted/successful**: the applicant finished the submission flow ending on the success page (`event == 'ApplicantAccessedSuccessPage'`). 
...
df_mp <- paths |> 
  str_subset("2025-11") |> 
  read_mp() 
# df_mp <- paths |> map(read_mp) |> list_rbind()
```
...
#| output: true
 ...
 df_compl_pilot <- df_mp |> 
  ...
  mutate(`Completion Rate` = `Accessed Success Page` / `Viewed Agreement`)
...
df_success_diff <- df_mp |> 
  filter(
    event %in% c("ApplicantAccessedSuccessPage"),
    pilot == "Nov 2025"
    ) |> 
  group_by(pilot) |>
  summarize(
    applicants = n_distinct(distinct_id),
    flows = n_distinct(distinct_id, cbv_flow_id),
    .groups = 'drop'
  )
```
Interestingly, there are a number of applicants who had multiple successful submissions (across multiple sessions, i.e. unique `cbv_flow_id`). In the November pilot, for example, there were `{r} scales::label_comma()(df_success_diff$applicants)` distinct applicants who submissed compared with `{r} scales::label_comma()(df_success_diff$flows)` distinct flows/sessions. 

The table below details how many submissions were associated with an applicant.^[Since not all applicants are using tokenized link, there may in fact be more applicants successfully submitting multiple times]
...
df_mp |> 
  filter(event %in% c("ApplicantAccessedSuccessPage")) |> 
  ...
  adjust_row_padding() |> 
  add_labs(
    title = "Some users has multiple successful submissions" |> toupper(),
    subtitle = "the number of successful submissions an applicant had",
    caption = default_caption
  ) 
```
...
df_compl <- df_mp |> 
  ...
  mutate(completion_rate_wk = `Accessed Success Page` / `Viewed Agreement`) |> 
  group_by(pilot) |> 
...
v1 <- df_compl |> 
  ...
  geom_hline(
    # aes(yintercept = completion_rate_plt), 
    yintercept = df_compl_pilot$`Completion Rate`, 
...
df_device_origin_status <- df_mp |> 
  filter(
    ...
    event %in% c("ApplicantViewedAgreement", "ApplicantAccessedSuccessPage")
  ) |> 
  mutate(
    device_type = map_chr(
      properties,
     ...
     origin = map_chr(
      properties,
      ~ pluck(.x, "origin", .default = NA_character_)
    ),
    completed = event == "ApplicantAccessedSuccessPage"
  ) |> 
  mutate(
    across(c(device_type, origin), ~ ifelse(is.na(.), "unknown", .)),
  ...
  ungroup() |> 
  distinct(pilot, distinct_id, cbv_flow_id, device_type, origin, completed) 

```

When comparing the device type, we can see there is a preference for applicants initiating the process on their phones. The success rate is much higher on the phones than desktop, but both success rates are rather low. The other types of devices include tablets, phablets, and portable media players.
...
df_device_origin_status |>
  ...
  mutate(,
    ...
    `Accessed Success Page` = `TRUE`,
    `Completion Rate` = `Accessed Success Page` / `Viewed Agreement`
  ) |> 
  select(-c(`FALSE`, `TRUE`)) |> 
  arrange(desc(`Viewed Agreement`)) |> 
...
On the client side, the goal of the IVaaS app is to creating something easy to complete. Ideally, applicants can complete the process in a matter of minutes. We can look at time it takes to submit to understand if it places an undue burden on applicants. 
...
Of the `{r} scales::label_comma()(df_duration_stats[df_duration_stats$completed_type == 'completed',]$n)` applicants who completed the application, the median time was `{r} scales::label_number(.1)(df_duration_stats[df_duration_stats$completed_type == 'completed',]$median)` minutes, with 75% completing it within `{r} scales::label_number(.1)(df_duration_stats[df_duration_stats$completed_type == 'completed',]$q75)` minutes. The timing looks pretty good from that vantage point. However, most applicants are not completing the application, `{r} scales::label_comma()(df_duration_stats[df_duration_stats$completed_type != 'completed',]$n |> sum())` distinct ids.^[Its important to note the switch here from applicants to distinct ids. Since most many applicants are sent generic links, we cannot track multiple sessions over time. So a user may start the process multiple times and complete and each one of those sessions would have its own distinct id] Of those distinct ids, `{r} scales::label_percent()(df_duration_stats[df_duration_stats$completed_type == 'just clicked',]$n / df_duration_stats[df_duration_stats$completed_type != 'completed',]$n |> sum())` are clicking the link and not proceeding beyond viewing the initial agreement page. When looking at incomplete applications excluding those that only viewed the agreement page, the median applicant spends `{r} scales::label_number(.1)(df_duration_stats[df_duration_stats$completed_type == 'incomplete',]$median)` minutes on the applicantion before they stop, with 75% aborting after only `{r} scales::label_number(.1)(df_duration_stats[df_duration_stats$completed_type == 'incomplete',]$q75)` minutes. 
...
df_duration_stats |> 
  mutate(
    ...
    completed_type = str_glue('{completed_type} (n = {label_comma()(n)})')) |> 
  ...
  facet_grid(pilot ~. , switch = "y") +
  scale_x_continuous(position = "top") +
  scale_color_identity() +
  labs(
    x = NULL, y = NULL,
    title = str_glue("USERS NOT COMPLETING THE APPLICATION END AFTER ABOUT {label_number(.1)(df_duration_stats[df_duration_stats$completed_type == 'incomplete',]$median)} MINUTES"),
    subtitle = 
      "displaying the median session **minutes** and IQR",
    caption = str_glue("Note: Applicants falling into 'just clicked' count represent those who didn't have an event beyond viewing the agreement 
    default_caption") 
      ) +
  si_style() +
  theme(
...
```
Overall, we are looking at a median sync time of `{r} median(df_sync$sync_time, na.rm = TRUE)` seconds, with 75% of users seeing it take under `{r} quantile(df_sync$sync_time, .75, na.rm = TRUE)` seconds. Of the `{r} scales::label_comma()(nrow(df_sync))` successful data syncs, `{r} v_lng_run_shr` take longer than `{r} lng_run` seconds to sync. 
...
df_outage_viz_rt <- df_sync_init |> 
  ...
  df_outage_viz_rt |> 
    ...
    summarize(
      ...
      success_rate = sum(succeeded)/sum(attempted))
...
df_outage_viz_rt |> 
  ...
  labs(
    ...
    subtitle = str_glue("daily <span style = color:{dsac_color['light_navy']};>**Argyle**</span>/<span style = color:{dsac_color['light_teal']};>**Pinwheel**</span> user flow attempts to login [left] and  successful rates [right]"),
    caption = str_glue('Note: Argyle outage Nov 19-20
    {default_caption}')
  ) + 
  si_style_xgrid() +
...
df_outage_viz |> 
  ...
  scale_x_continuous(limit=c(-45,300),oob=scales::squish, position = "top") + 
  scale_color_identity() +
  scale_size(guide = "none") +
  labs(
    x = NULL, y = NULL,
    title = str_glue("DESPITE SOME LARGE OUTLIERS AND LESS FLOWS, PINWHEEL'S MEDIAN SYNC TIME ({df_outage_provider_median$Pinwheel}s) IS ONLY A FEW SECONDS SLOWER THAN ARGYLE ({df_outage_provider_median$Argyle}s)") |> str_wrap(),
    subtitle = str_glue("successful daily <span style = color:{dsac_color['light_navy']};>**Argyle**</span>/<span style = color:{dsac_color['light_teal']};>**Pinwheel**</span> user flow syncs [left] and their duration, **seconds** [median and IQR]"),
    caption = str_glue('Note: x axis truncated to 5 minutes; Argyle outage Nov 19-20 
    {default_caption}')
  ) + 
  si_style_xgrid() +
  theme(plot.subtitle = element_markdown())
...
df_ret <- df_mp |>
  ...
  clean_events() |> 
  select(distinct_id, event_clean) |> 
  distinct(distinct_id, event_clean) |> 
  count(event_clean, sort = TRUE, name = "views") |> 
  mutate(
    share_lag = views / lag(views),
    share_cum = views / max(views),
    share_cum = ifelse(event_clean == "Viewed Agreement", NA, share_cum)
      )
