...
df_compl <- df_mp |> 
  ...
  mutate(completion_rate_plt = sum(`Accessed Success Page`) / sum(`Viewed Agreement`)) |> 
  ungroup()
```
...
df_compl |> 
  group_by(pilot) |> 
  summarise(
    across(c(`Viewed Agreement`, `Accessed Success Page`), 
    \(x) sum(x, na.rm = TRUE)),
   .groups = "drop"
    ) |> 
  mutate(`Completion` = `Accessed Success Page` / `Viewed Agreement`) |> 
  gt() |> 
  format_numeric_columns() |> 
  si_gt_base(header_fill = dsac_color['teal']) |> 
  adjust_row_padding() |> 
  add_labs(
...
v1 <- df_compl |> 
  pivot_longer(
    c(`Accessed Success Page`, `Viewed Agreement`),
    names_to = "event") |> 
  ggplot(aes(plt_wk, value)) +
  geom_col(fill = dsac_color[['teal']]) +
  facet_grid(fct_rev(event) ~ pilot, scales = "free_x", switch = "y") +
  scale_y_continuous(label = label_comma()) +
  si_style_ygrid() +
  labs(
    x = NULL, y = NULL,
    subtitle = "Views by Applicant") +
  theme(
  ...
  theme(
    ...
    v1 / v2 + 
      plot_annotation(
        title = "Large drop off in Views after the first week, but larger completion rate in following weeks" |> toupper(),
        caption = default_caption,
        theme = si_style()
      )
```
...
On the client side, the goal of the app is to creating something easy to complete. Ideally, applicants can complete the process in a matter of minutes. We can look at time it takes to submit to understand if it places an undue burden on applicants. 
...
df_duration_stats |> 
  ...
  labs(
    ...
    subtitle = 
      "displaying the median session **minutes** against the range from the 25-75 percentile",
    caption = default_caption 
      ) +
  si_style() +
  theme(
...
```
Overall, we are looking at a median sync time of `{r} median(df_sync$sync_time, na.rm = TRUE)` seconds, with 75% of users seeing it take under `{r} quantile(df_sync$sync_time, .75, na.rm = TRUE)` seconds. Of the `scales::label_comma()(nrow(df_sync))` successful data syncs, `{r} v_lng_run_shr` take longer than `{r} lng_run` seconds to sync. 
...
df_sync_viz |> 
  ...
  labs(
    x = NULL, y = NULL,
    title = "Slightly shortern median sync for Pinwheel, but a much small sample size" |> toupper(),
    subtitle = 
      "displaying the median session **seconds** against the range from the 25-75 percentile",
    caption = default_caption 
      ) +
  si_style_xgrid() +
  theme(
...
df_outage_viz_rt <- df_sync_init |> 
  ...
  mutate(
    date = ApplicantAttemptedLogin |> as_date(),
    succeeded = !is.na(ApplicantFinishedSync)
  ) |>
  group_by(date, provider) |> 
  summarize(
    attempted = distinct_n(distinct_id),
    succeeded = sum(!is.na(ApplicantFinishedSync), na.rm = TRUE),
    .groups = "drop"
  ) |> 
  mutate(
    ...
    date_fmt = date |> format(date, format = "%b %d") |> str_remove(" UTC") |> fct_inorder()
  )

  df_outage_viz_rt |> 
    group_by(provider) |> 
    summarize(success_rate = sum(succeeded)/sum(attempted))

df_outage_viz_rt |> 
  ggplot(aes(y = fct_rev(date_fmt), color = fillcolor, group = date_fmt)) +
  geom_linerange(aes(xmin = 0, xmax = success_rate), color = '#909090') +
  geom_point(aes(success_rate)) +
  ...
  labs(
    x = NULL, y = NULL,
    subtitle = str_glue("daily <span style = color:{dsac_color['light_navy']};>**Argyle**</span> to <span style = color:{dsac_color['light_teal']};>**Pinwheel**</span> user attempts to login [left] and  successful rates [right]"),
    caption = str_glue('{default_caption}')
  ) + 
  si_style_xgrid() +
  theme(plot.subtitle = element_markdown())

si_save("success_rate.png")
...
df_outage_viz |> 
  ...
  labs(
    x = NULL, y = NULL,
    subtitle = str_glue("daily <span style = color:{dsac_color['light_navy']};>**Argyle**</span> to <span style = color:{dsac_color['light_teal']};>**Pinwheel**</span> users [left] and their successful sync times, seconds [median and IQR]"),
    caption = str_glue('Note: x axis truncated to 5 minutes 
    {default_caption}')
  ) + 
  si_style_xgrid() +
  theme(plot.subtitle = element_markdown())

si_save("sync_times.png")
```
