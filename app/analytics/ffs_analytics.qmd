...
df_duration_viz |> 
  ...
  labs(
    x = NULL, y = NULL,
    title = glue("{toupper(df_duration_stats$pilot[1])} INCOMPLETE APPLICATIONS END AFTER ABOUT {label_number(.1)(df_duration_stats[df_duration_stats$completed_type == 'incomplete',]$median)} MINUTES"),
    subtitle = 
...
df_sankey |> 
  ...
  scale_x_discrete(labels = v_steps_clean[2:8], position = "top") +
  scale_fill_identity() + 
  coord_cartesian(clip = 'off') +
  labs(
    x = NULL, y = NULL,
...
df_sessions <- df_time |> 
  ...
  summarise(
    sessions = n_distinct(cbv_flow_id),
    started = max(started, na.rm = TRUE),
    completed = max(completed, na.rm = TRUE),
  ...
  summarise(
    n_users = sum(started, na.rm = TRUE),
