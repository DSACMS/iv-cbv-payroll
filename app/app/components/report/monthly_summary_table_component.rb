class Report::MonthlySummaryTableComponent < ViewComponent::Base
  ...
  extend Report::MonthlySummaryTableHelper
  ...
  def partial_month_details(current_month_string, activity_dates, report_from_date, report_to_date)
    ...
    ## String comparisons of current month to report month range
      ...
      { is_partial_month: earliest_activity != start_of_month, description: partial_month_description(earliest_activity, end_of_month), included_range_start: earliest_activity, included_range_end: end_of_month }
    elsif current_month_string == self.format_month(report_to_date)
      latest_activity = activity_dates.max
      { is_partial_month: latest_activity != end_of_month,  description: partial_month_description(start_of_month, latest_activity), included_range_start: start_of_month, included_range_end: latest_activity }
    else
      { is_partial_month: false, description: nil, included_range_start: start_of_month, included_range_end: end_of_month }
    end
  end
