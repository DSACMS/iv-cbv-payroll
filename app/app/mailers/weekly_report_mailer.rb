...
class WeeklyReportMailer < ApplicationMailer
  ...
  def report_email
    ...
    raise "Invalid `client_agency_id` parameter given: #{params[:client_agency_id].inspect}" unless current_client_agency.present?

    raise "Missing `report_date` param" unless params[:report_date].present?
    now = params[:report_date]

    raise "Missing `weekly_report.recipient` configuration for client agency: #{params[:client_agency_id]}" unless current_client_agency.weekly_report["recipient"]
    @recipient = current_client_agency.weekly_report["recipient"]

    @report_range = now.prev_week.all_week
    csv_rows = weekly_report_data(current_client_agency, @report_range)
  ...
  def weekly_report_data(current_client_agency, report_range)
    CbvFlowInvitation
      .where(client_agency_id: current_client_agency.id)
