class ClientAgency::AzDes::ReportDelivererJob < ApplicationJob
  def perform(date_start, date_end)
    cbv_applicants_delivered_recently = CbvFlow.where(transmitted_at: date_start..date_end).where(client_agency_id: ClientAgency::AzDes::Configuration.client_agency_id)
