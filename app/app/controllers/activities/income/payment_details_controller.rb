class Activities::Income::PaymentDetailsController < Activities::BaseController
  ...
  def show
    ...
    if @payroll_account.nil?
      return redirect_to(activities_flow_entry_url, flash: { slim_alert: { message: t("activities.income.error_no_access"), type: "error" } })
    end
    ...
    @account_comment = @payroll_account.additional_information
  ...
  def employer_name
    return I18n.t("activities.income.payment_details.show.unknown") unless has_employment_data?
  ...
  def pay_frequency
    return I18n.t("activities.income.payment_details.show.unknown") unless has_income_data?
  ...
  def gross_pay
    return I18n.t("activities.income.payment_details.show.unknown") unless has_paystubs_data?
  ...
  def track_saved_event
    comment_data = @payroll_account&.additional_information
    ...
    event_logger.track(TrackEvent::ApplicantSavedPaymentDetails, request, {
      ...
      client_agency_id: current_agency&.id,
      invitation_id: @flow.invitation_id,
      additional_information_length: comment_data ? comment_data.length : 0
    })
  end

  def payroll_account_params
    params.require(:payroll_account).permit(:additional_information)
  end
end
