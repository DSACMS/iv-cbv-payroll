  ...
  def process_webhook_event
    if @webhook_event.event_name == "account.added"
      ...
      event_logger.track(TrackEvent::ApplicantFinishedPinwheelSync, request, {
        ...
        identity_emails_count: report.identities.sum { |i| i.emails&.length.to_i },
        identity_phone_numbers_count: report.identities.sum { |i| i.phone_numbers&.length.to_i },
        ...
        paystubs_deductions_count: report.paystubs.sum { |p| p.deductions&.length.to_i },
        paystubs_hours_average: paystub_hours.sum.to_f / paystub_hours.length,
        paystubs_hours_by_earning_category_count: report.paystubs.sum { |p| p.hours_by_earning_category&.length.to_i },
        ...
        paystubs_earnings_count: report.paystubs.sum { |p| p.earnings&.length.to_i },
        paystubs_earnings_with_hours_count: report.paystubs.sum { |p| p.earnings&.count { |e| e.hours.present? }.to_i },
        paystubs_earnings_category_salary_count: report.paystubs.sum { |p| p.earnings&.count { |e| e.category == "salary" }.to_i },
        paystubs_earnings_category_bonus_count: report.paystubs.sum { |p| p.earnings&.count { |e| e.category == "bonus" }.to_i },
        paystubs_earnings_category_overtime_count: report.paystubs.sum { |p| p.earnings&.count { |e| e.category == "overtime" }.to_i },
        paystubs_earnings_category_tips_count: report.paystubs.sum { |p| p.earnings&.count { |e| e.category == "tips" }.to_i },
