...
RSpec.describe Cbv::PinwheelDataHelper, type: :helper do
  ...
  let(:payments) do
    raw_payments_json = pinwheel_load_relative_json_file('request_end_user_paystubs_response.json')['data']
  ...
  let(:employment) do
    ResponseObjects::Employment.from_pinwheel(pinwheel_load_relative_json_file('request_employment_info_response.json')['data'])
  end

  let(:incomes) do
    ResponseObjects::Income.from_pinwheel(pinwheel_load_relative_json_file('request_income_metadata_response.json')['data'])
  end

  let(:identities) do
    ResponseObjects::Identity.from_pinwheel(pinwheel_load_relative_json_file('request_identity_response.json')['data'])
  end
