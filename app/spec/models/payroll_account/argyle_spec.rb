...
RSpec.describe PayrollAccount::Argyle, type: :model do
  ...
  describe '.available_jobs' do
    it 'returns all job types from JOBS_TO_WEBHOOK_EVENTS' do
      expect(described_class.available_jobs).to match_array([ 'identities', 'paystubs', 'gigs' ])
    end
  end
  ...
  describe '#has_fully_synced?' do
    ...
    it 'handles unsupported jobs correctly' do
      account = create(:payroll_account, :argyle, cbv_flow: cbv_flow, supported_jobs: [ 'identities' ])
      create(:webhook_event, payroll_account: account, event_name: 'identities.added')

      expect(account.has_fully_synced?).to be true
    end
  end

  describe '#job_succeeded?' do
    it 'returns true when the job succeeded' do
      expect(synced_account.job_succeeded?('identities')).to be true
    end
  ...
  describe '#synchronization_status' do
    ...
    it 'returns :succeeded when job succeeded' do
      expect(synced_account.synchronization_status('identities')).to eq(:succeeded)
    end
  ...
  describe '#has_required_data?' do
    it 'returns true when paystubs succeeded' do
      ...
      create(:webhook_event, payroll_account: account, event_name: 'paystubs.added', event_outcome: 'success')
