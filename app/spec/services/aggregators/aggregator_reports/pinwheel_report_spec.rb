...
RSpec.describe Aggregators::AggregatorReports::PinwheelReport, type: :service do
  ...
  let(:shifts_json) { load_relative_json_file('request_end_user_shifts_response.json') }
  ...
  describe '#fetch' do
    ...
    it 'transforms all response objects correctly' do
      ...
      expect(service.instance_variable_get(:@gigs)).to all(be_an(Aggregators::ResponseObjects::Gig))
    end
