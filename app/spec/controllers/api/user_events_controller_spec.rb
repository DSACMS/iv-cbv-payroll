...
RSpec.describe Api::UserEventsController, type: :controller do
  ...
  describe "#user_action" do
    ...
    context "when tracking a LearnHowItWorksClicked event" do
      let(:event_name) { "LearnHowItWorksClicked" }
      let(:event_attributes) { {} }

      it "tracks an event with Mixpanel" do
        expect(EventTrackingJob).to receive(:perform_later).with("LearnHowItWorksClicked", anything, hash_including(
          time: be_a(Integer),
          cbv_flow_id: cbv_flow.id,
          invitation_id: cbv_flow.cbv_flow_invitation_id
        ))
