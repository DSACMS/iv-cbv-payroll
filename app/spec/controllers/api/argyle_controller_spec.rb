  ...
  describe "#create" do
    ...
    context "when the payroll account or employer has already been linked" do
      ...
      context "if the payroll sync is in progress" do
        ...
        before do
          # Update the aggregator_account_id to match the one returned by fetch_accounts_api
          payroll_account.update(aggregator_account_id: argyle_account_id)
        end
        ...
        context "after receiving the accounts.connected webhook" do
          ...
          it "redirects to the synchronizations page" do
            ...
            expect(response).to redirect_to(cbv_flow_synchronizations_path(user: { account_id: payroll_account.aggregator_account_id }))
          end
        end
      end
