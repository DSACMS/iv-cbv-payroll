...
RSpec.describe ApplicationController, type: :controller do
  ...
  describe "#session_timeout_duration" do
    ...
    context "when no launcher timeout is set in session" do
      it "returns the default timeout" do
        expect(controller.send(:session_timeout_duration)).to eq(default_timeout)
      end
    end

    context "when launcher_timeout is set in session" do
      before do
        session[:launcher_timeout] = 10.minutes.to_i
      end

      it "returns the launcher timeout" do
